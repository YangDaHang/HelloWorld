...
% Last Modified by GUIDE v2.5 22-Oct-2016 23:07:53
...
v2_max=max(v2);
X=handles.data(number).rex_dr_range(1,2:22);       %2000 2100......4000
Y=handles.data(number).rex_dr_range(1+v2_max:11,1);
Z=handles.data(number).dr(v2_max:10,1:21);

x=handles.data(number).rex_dr_range(1,2:22);
y=handles.data(number).rex_dr_range(1+v2_max:11,1);
z=handles.data(number).dr(v2_max:10,1:21);
axes(handles.axes4);
cla;
...
shading faceted;
hold on;
[c,h]=contour3(X,Y,Z,5,'w','linewidth',3);    
clabel(c);
